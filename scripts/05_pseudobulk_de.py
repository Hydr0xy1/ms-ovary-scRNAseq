...
def aggregate_cell_type(
    ...
    *,
    ...
    sample_to_group = adata.obs.groupby(sample_key, observed=True)["group"].first().astype(str)
...
def rescue_table(
    ...
    merged.loc[eligible & merged["opposite_direction"] & merged["closer_to_y"], "rescue_class"] = (
...
def main() -> None:
    ...
    for key in (settings["sample_key"], settings["group_key"], settings["cell_type_key"]):
        if key not in adata.obs:
            raise KeyError(f"Required obs column missing: {key}")
    if counts_layer not in adata.layers:
        raise KeyError(f"Required layer missing: {counts_layer}")

    cell_counts = pd.crosstab(adata.obs[settings["sample_key"]], adata.obs[settings["cell_type_key"]])
    output_root = paths["results"] / "05_pseudobulk"
    output_root.mkdir(parents=True, exist_ok=True)
    cell_counts.to_csv(output_root / "cells_per_sample_cell_type.tsv", sep="\t")

    group_by_sample = adata.obs.groupby(settings["sample_key"], observed=True)[settings["group_key"]].first()
    eligible_cell_types: list[str] = []
    for cell_type in cell_counts.columns.astype(str):
        eligible_samples = cell_counts.index[cell_counts[cell_type] >= int(settings["min_cells_per_sample_cell_type"])]
        ...
        if all(group_counts.get(group, 0) >= int(settings["min_samples_per_group"]) for group in required_groups):
            eligible_cell_types.append(cell_type)
        else:
            logger.warning("Skipping %s: insufficient eligible sample replicates %s", cell_type, group_counts.to_dict())
    ...
    for cell_type in eligible_cell_types:
        ...
        keep_samples = cell_counts.index[cell_counts[cell_type] >= int(settings["min_cells_per_sample_cell_type"])]
